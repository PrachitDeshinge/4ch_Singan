...
def move_to_gpu(t, device):
    """Safely move tensor to device with proper dtype handling"""
    if device.type == 'mps':
...
def get_device(opt):
    """Get appropriate device based on availability and user preferences"""
    if opt.not_cuda:
        return torch.device('cpu')
    elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
        return torch.device('mps')
    elif torch.cuda.is_available():
        return torch.device(f'cuda:{opt.gpu_id}' if opt.gpu_id else 'cuda:0')
    return torch.device('cpu')
...
def np2torch(x, opt):
    ...
    x = x.type(torch.float32)  # Convert to float32 first
    if not opt.not_cuda:
...
def load_trained_pyramid(opt, mode_='train'):
    # Add allowed globals for safe weights-only loading.
    ...
    if(os.path.exists(dir)):
        Gs = torch.load('%s/Gs.pth' % dir, weights_only=True)
        Zs = torch.load('%s/Zs.pth' % dir, weights_only=True)
        reals = torch.load('%s/reals.pth' % dir, weights_only=True)
        NoiseAmp = torch.load('%s/NoiseAmp.pth' % dir, weights_only=True)
    else:
...
def post_config(opt):
    # Initialize device, seed, etc.
    opt.device = get_device(opt)
    print(f"Selected device: {opt.device}")
    ...
    elif torch.backends.mps.is_available():  # Check for Mac M1/M2
    ...
    # REMOVE the fixed assignment and use the directory generator for consistency.
    # Old line:
