...
def train(opt,Gs,Zs,reals,NoiseAmp):
    import os
    checkpoint_path = os.path.join(opt.out_, 'checkpoint.pth')
    # Load checkpoint if resume flag is true and file exists
    if opt.resume and os.path.exists(checkpoint_path):
        ...
        scale_num = checkpoint['scale_num']
        in_s = checkpoint['in_s']
        print("Resuming training from scale:", scale_num)
    else:
        scale_num = 0
        in_s = 0
    nfc_prev = 0
    ...
    print("Full pyramid computed. Resuming at scale:", scale_num)  # Debug print
    ...
    try:
        while scale_num<opt.stop_scale+1:
            ...
            if (nfc_prev==opt.nfc):
                G_curr.load_state_dict(torch.load('%s/%d/netG.pth' % (opt.out_,scale_num-1), weights_only=True))
                D_curr.load_state_dict(torch.load('%s/%d/netD.pth' % (opt.out_,scale_num-1), weights_only=True))
            ...
            print("Complete save: Zs, Gs, reals, and NoiseAmp saved in", opt.out_)
            
            # Increment scale number before saving checkpoint so resume starts at the next scale
            scale_num += 1
            cp_path = os.path.join(opt.out_, 'checkpoint.pth')
            torch.save({
                'Gs': Gs,
                'Zs': Zs,
                'reals': reals,
            ...
            print("Checkpoint saved to", cp_path)

            nfc_prev = opt.nfc
            del D_curr,G_curr
        return
    except KeyboardInterrupt as e:
        # Use a backup value if scale_num is not defined in this context
        local_scale = scale_num if 'scale_num' in locals() else 0
        cp_path = os.path.join(opt.out_, 'checkpoint.pth')
        torch.save({
            'Gs': Gs,
            'Zs': Zs,
            'reals': reals,
            'NoiseAmp': NoiseAmp,
            'scale_num': local_scale,
            'in_s': in_s
        }, cp_path)
        print("KeyboardInterrupt caught. Checkpoint saved to", cp_path)
        raise e
